...
const loadLikedSongsAndDispatch = async (action, dispatch) => {
  ...
  const q = query(collection(db, USERSCOLLECTION, userID, SONGCOLLECTION));
...
const deleteLikedSongsAndDispatch = async (action, dispatch) => {
  ...
  console.log(
    "=======This is my action in DB.js when i try to delete a song======="
  );
  console.log(action);
...
const loadYourSongsAndDispatch = async (action, dispatch) => {
  ...
  snapshotUnsubscribe = onSnapshot(q, (qSnap) => {
    ...
    let filteredYourSongs = allYourSongs.filter(
      (elem) => elem.postedBy === "me"
    );
...
const loadFeedAndDispatch = async (action, dispatch) => {
  const querySnap = await getDocs(collection(db, FEEDCOLLECTION));
  let newFeed = [];
  querySnap.forEach((docSnap) => {
    let newPost = docSnap.data();
    newPost.key = docSnap.id;
